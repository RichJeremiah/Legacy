...
<div>
    <p>There are currently <?php echo $warband_manager->get_current_member_count() ?> users recorded as members</p>
    <?php if ($warband_manager->is_warband_open()) : ?>
        <p>Your Warband is currently open for anyone to join &nbsp;
            <span><button value="Make Private" class="wb-make-private btn-sm"
                          data-name=" <?php echo $warband_manager->get_warband_title() ?>"
                          data-wbid="<?php echo $warband_manager->get_warband_id() ?>">Make Private</button>
            </span>
        </p>
    <?php else : ?>
    <p>Membership of your warband is by request and approval only. &nbsp;
        <span><button value="Make Public" class="wb-make-public btn-sm"
                      data-name=" <?php echo $warband_manager->get_warband_title() ?>"
                      data-wbid="<?php echo $warband_manager->get_warband_id() ?>">Make Public</button></span></p>
    ...
    <ol>
        ...
        <li>In the Warband Code box enter the following code: <?php echo $warband_manager->get_warband_code() ?> (This is a
            unique
            code for the <?php echo $warband_manager->get_warband_title() ?> Warband)
        </li>
    </ol>
    They will then appear in the list below where you can choose to Approve or Reject their request </p>
    <p> There are currently <?php echo count($warband_manager->get_users_awaiting_approval()) ?> users awaiting approval </p>
</div>
<div class="warband-user-table ">
    ...
    <?php foreach ($warband_manager->get_users_awaiting_approval() as $user) : ?>
        <div id="req-<?php echo $user->ID ?>" class="row alert alert-fix alert-dismissible fade show" role="alert">
            <div class="col"> <?php echo $user->display_name ?></div>
            <div class="col">
                <button value="Approve" class="wb-user-approve btn btn-success"
                        data-id="<?php echo $user->ID ?>"
                        data-wbcode="<?php echo $warband_manager->get_warband_code() ?>"
                        data-wbid="<?php echo $warband_manager->get_warband_Id() ?>">Approve
                </button>
                <button value="Reject" class="wb-user-reject btn btn-danger"
                        data-id="<?php echo $user->ID ?>">Reject
                </button>
            </div>
        </div>
    <?php endforeach; ?>
